...
null,e=null,g=false;c.checkerFunction=function(){if(g){b.debug("hashchange.checker: checker is running");return false}g=true;var i=b.getHash(),j=b.normalizeHash(a.contentWindow.document.location.hash);if(i!==d){d=i;if(j!==i){b.debug("hashchange.checker: iframe hash change",j,i);e=i;a.contentWindow.document.open();a.contentWindow.document.close();a.contentWindow.document.location.hash=i}b.Adapter.trigger(h,"hashchange")}else if(j!==e){b.debug("hashchange.checker: iframe hash out of sync",j,i);e=j;
b.setHash(j)}g=false;return true}}else{d=null;c.checkerFunction=function(){var i=b.getHash();if(i!==d){d=i;b.Adapter.trigger(h,"hashchange")}return true}}setInterval(c.checkerFunction,b.options.hashChangeCheckerDelay);return true}();if(b.emulated.pushState){c.onHashChange=function(a){b.debug("_History.onHashChange",this,arguments);currentHash=unescape(b.extractHashFromUrl(a&&a.newURL||f.location.href));currentStateHashExits=currentStateHash=currentState=null;if(c.isLastHash(currentHash)){b.log("_History.onHashChange: no change");
...
currentState.title,currentState.url);return true};b.Adapter.bind(h,"hashchange",c.onHashChange);b.pushState=function(a,d,e){b.debug("History.pushState",this,arguments);if(b.extractHashFromUrl(e))throw Error("History.js does not support states with fragement-identifiers (hashes/anchors).");var g=b.createStateObject(a,d,e),i=b.createStateHash(g);b.getState();var j=b.getStateHash(),m=unescape(b.getHash());c.storeState(g);c.recycleState(g);if(g.title)f.title=g.title;b.log("History.pushState: details",
"newStateHash:",i,"oldStateHash:",j,"html4Hash:",m);if(i===j){b.log("History.pushState: no change");return false}if(i!==m){b.log("History.pushState: update hash");b.setHash(escape(i));return false}c.saveState(g);b.log("History.pushState: trigger popstate");b.Adapter.trigger(h,"statechange");return true};b.replaceState=function(a,d,e){b.log("History.replaceState",this,arguments);if(b.extractHashFromUrl(e))throw Error("History.js does not support states with fragement-identifiers (hashes/anchors).");
var g=b.createStateObject(a,d,e),i=b.getState(),j=c.getStateByIndex(-2);c.discardState(i,g,j);b.pushState(g.data,g.title,g.url);return true};if(!f.location.hash||f.location.hash==="#")b.Adapter.onDomLoad(function(){b.log("hash1");var a=b.createStateObject({},"",f.location.href);b.pushState(a.data,a.title,a.url)});else if(!b.emulated.hashChange){b.log("hash2");b.Adapter.onDomLoad(function(){c.onHashChange()})}}else{c.onPopState=function(a){b.debug("_History.onPopState",this,arguments);if(b.getHash())b.Adapter.trigger(h,
"anchorchange");else{var d={},e=null,g=null;d=null;a=a||{};if(typeof a.state==="undefined")if(typeof a.originalEvent!=="undefined"&&typeof a.originalEvent.state!=="undefined")a.state=a.originalEvent.state;if(a.state===null)d=a.originalEvent.state;else if(typeof a.state!=="undefined"){e=b.expandUrl(f.location.href);d=c.getStateByUrl(e);e=c.urlDuplicateExists(e);d=typeof d!=="undefined"&&!e?d.data:a.originalEvent.state}else{e=b.expandUrl(f.location.href);d=c.getStateByUrl(e);if(e==d.url)d=d.data;else throw Error("Unknown state");
}d=typeof d!=="object"||d===null?{}:d;e=d.title||"";g=d.url||f.location.href;d=b.createStateObject(d,e,g);if(c.isLastState(d)){b.log("_History.onPopState: no change");return false}c.storeState(d);c.saveState(d);if(d.title)f.title=d.title;b.Adapter.trigger(h,"statechange");return true}};b.Adapter.bind(h,"popstate",c.onPopState);b.pushState=function(a,d,e){if(b.extractHashFromUrl(e))throw Error("History.js does not support states with fragement-identifiers (hashes/anchors).");a=b.createStateObject(a,
d,e);c.storeState(a);k.pushState(a.data,a.title,a.url);b.Adapter.trigger(h,"popstate");return true};b.replaceState=function(a,d,e){if(b.extractHashFromUrl(e))throw Error("History.js does not support states with fragement-identifiers (hashes/anchors).");a=b.createStateObject(a,d,e);c.storeState(a);k.replaceState(a.data,a.title,a.url);b.Adapter.trigger(h,"popstate");return true}}b.back=function(){b.debug("History.back: called");if(b.emulated.hashChange&&c.isInternetExplorer()){var a=b.getHash();setTimeout(function(){if(b.getHash()===
a){b.debug("History.back: trying again");return b.back()}return true},b.options.hashChangeCheckerDelay*2)}return k.go(-1)};b.forward=function(){b.debug("History.forward: called");if(b.emulated.hashChange&&c.isInternetExplorer()){var a=b.getHash();setTimeout(function(){if(b.getHash()===a){b.debug("History.forward: trying again");return b.forward()}return true},b.options.hashChangeCheckerDelay*2)}return k.go(1)}};typeof b.Adapter!=="undefined"&&b.init()})(window);
