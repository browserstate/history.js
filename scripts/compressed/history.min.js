...
return false}c.saveHash(currentHash);try{currentState=JSON.parse(currentHash)}catch(d){b.debug("_History.onHashChange: traditional anchor");b.Adapter.trigger("anchorchange");return false}if(c.isLastState(currentState)){b.debug("_History.onHashChange: no change");return false}currentStateHash=b.createStateHash(currentState);b.debug("_History.onHashChange: ","currentStateHash",currentStateHash,"Hash -1",c.getHashByIndex(-1),"Hash -2",c.getHashByIndex(-2),"Hash -3",c.getHashByIndex(-3),"Hash -4",c.getHashByIndex(-4),
"Hash -5",c.getHashByIndex(-5),"Hash -6",c.getHashByIndex(-6),"Hash -7",c.getHashByIndex(-7));var e=c.discardedState(currentState);if(e){b.debug("forwardState:",b.createStateHash(e.forwardState),"backState:",b.createStateHash(e.backState));if(c.getHashByIndex(-2)===b.createStateHash(e.forwardState)){b.debug("_History.onHashChange: go backwards");b.back()}else{b.debug("_History.onHashChange: go forwards");b.forward()}return false}b.debug("_History.onHashChange: success hashchange");b.pushState(currentState.data,
...
"newStateHash:",h,"oldStateHash:",j,"html4Hash:",m);if(h===j){b.debug("History.pushState: no change");return false}if(h!==m){b.debug("History.pushState: update hash");b.setHash(escape(h));return false}c.saveState(g);b.debug("History.pushState: trigger popstate");b.Adapter.trigger(i,"statechange");return true};b.replaceState=function(a,d,e){b.debug("History.replaceState",this,arguments);if(b.extractHashFromUrl(e))throw Error("History.js does not support states with fragement-identifiers (hashes/anchors).");
...
if(d){try{var e=JSON.parse(d);b.debug("_History.onPopState: state anchor",d,e);b.replaceState(e.data,e.tite,e.url)}catch(g){b.debug("_History.onPopState: traditional anchor",d);b.Adapter.trigger(i,"anchorchange")}return false}d={};var h=e=null;d=null;a=a||{};if(typeof a.state==="undefined")if(typeof a.originalEvent!=="undefined"&&typeof a.originalEvent.state!=="undefined")a.state=a.originalEvent.state;else if(typeof a.event!=="undefined"&&typeof a.event.state!=="undefined")a.state=a.event.state;if(a.state===
null)d=a.state;else if(typeof a.state!=="undefined"){e=b.expandUrl(f.location.href);d=c.getStateByUrl(e);e=c.urlDuplicateExists(e);d=typeof d!=="undefined"&&!e?d.data:a.state}else{e=b.expandUrl(f.location.href);d=c.getStateByUrl(e);if(e==d.url)d=d.data;else throw Error("Unknown state");}d=typeof d!=="object"||d===null?{}:d;e=d.title||"";h=d.url||f.location.href;d=b.createStateObject(d,e,h);if(c.isLastState(d)){b.debug("_History.onPopState: no change");return false}b.debug("_History.onPopState","newState:",
...
return b.back()}return true},b.options.hashChangeCheckerDelay*2)}return k.go(-1)};b.forward=function(){b.debug("History.forward: called");if(b.emulated.hashChange&&c.isInternetExplorer()){var a=b.getHash();setTimeout(function(){if(b.getHash()===a){b.debug("History.forward: trying again");return b.forward()}return true},b.options.hashChangeCheckerDelay*2)}return k.go(1)};b.go=function(a){b.debug("History.go: called with index ["+a+"]");if(a>0)for(var d=0;d<a;++d)setTimeout(b.forward,b.options.hashChangeCheckerDelay*
4*d);else if(a<0)for(d=0;d>a;--d)setTimeout(b.back,b.options.hashChangeCheckerDelay*4*d*-1);else throw Error("History.go: History.go requires a positive or negative integer passed.");return true}};typeof b.Adapter!=="undefined"&&b.init()})(window);
